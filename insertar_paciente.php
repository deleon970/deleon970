...
if ($stmt_check->num_rows > 0) {
    ...
    $check_duplicate = "SELECT * FROM pacientes WHERE nombre = ? AND nombre_propietario = ?";
    ...
    if ($stmt_duplicate->num_rows > 0) {
        ...
        $sql = "INSERT INTO pacientes (nombre_propietario, nombre, especie, sexo, peso, edad, id_cita) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssi", $nombre_propietario, $nombre, $especie, $sexo, $peso, $edad, $id_cita);
